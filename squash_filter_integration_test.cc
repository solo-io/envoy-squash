...
public:
   ...
   */
  void SetUp() override {
    fake_upstreams_.emplace_back(new Envoy::AutonomousUpstream(0, Envoy::FakeHttpConnection::Type::HTTP1, version_));
  ...
  Envoy::IntegrationStreamDecoderPtr sendDebugRequest(Envoy::IntegrationCodecClientPtr& codec_client) {
    Envoy::IntegrationStreamDecoderPtr response(new Envoy::IntegrationStreamDecoder(*dispatcher_));
    Envoy::Http::TestHeaderMapImpl headers{{":method", "GET"}, {":authority", "www.solo.io"}, {"x-squash-debug", "true"}, {":path", "/getsomething"}};
    codec_client->makeHeaderOnlyRequest(headers, *response);
    return response;
  }

  Envoy::FakeStreamPtr sendSquash(Envoy::FakeHttpConnectionPtr& fake_squash_connection, std::string status, std::string body) {
  ...
  Envoy::FakeStreamPtr sendSquashCreate(Envoy::FakeHttpConnectionPtr& fake_squash_connection, std::string body) {
    return sendSquash(fake_squash_connection, "201", body);
  }
  
  Envoy::FakeStreamPtr sendSquashOk(Envoy::FakeHttpConnectionPtr& fake_squash_connection, std::string body) {
...
TEST_P(SquashFilterIntegrationTest, Test1) {
  ...
  Envoy::FakeHttpConnectionPtr fake_squash_connection = fake_upstreams_[1]->waitForHttpConnection(*dispatcher_);

  // respond to create request
  Envoy::FakeStreamPtr create_stream = sendSquashCreate(fake_squash_connection, "{\"metadata\":{\"name\":\"oF8iVdiJs5\"},\"spec\":{\"attachment\":{\"a\":\"b\"},\"image\":\"debug\",\"node\":\"debug-node\"},\"status\":{\"state\":\"none\"}}");
  /*
  if (fake_upstreams_[1]->httpType() == Envoy::FakeHttpConnection::Type::HTTP1) {
    fake_squash_connection->waitForDisconnect();
    fake_squash_connection = fake_upstreams_[1]->waitForHttpConnection(*dispatcher_);
  } else {
    create_stream->waitForReset();
  }
*/
  // respond to read attachment request
  Envoy::FakeStreamPtr get_stream = sendSquashOk(fake_squash_connection, "{\"metadata\":{\"name\":\"oF8iVdiJs5\"},\"spec\":{\"attachment\":{\"a\":\"b\"},\"image\":\"debug\",\"node\":\"debug-node\"},\"status\":{\"state\":\"attached\"}}");
...
TEST_P(SquashFilterIntegrationTest, ErrorAttaching) {
   ...
   // respond to read attachment request with error!
    Envoy::FakeStreamPtr get_stream = sendSquashOk(fake_squash_connection, "{\"metadata\":{\"name\":\"oF8iVdiJs5\"},\"spec\":{\"attachment\":{\"a\":\"b\"},\"image\":\"debug\",\"node\":\"debug-node\"},\"status\":{\"state\":\"error\"}}");
  ...
  TEST_P(SquashFilterIntegrationTest, TimeoutAttaching) {
      ...
      // respond to create request
      Envoy::FakeStreamPtr create_stream = sendSquashCreate(fake_squash_connection, "{\"metadata\":{\"name\":\"oF8iVdiJs5\"},\"spec\":{\"attachment\":{\"a\":\"b\"},\"image\":\"debug\",\"node\":\"debug-node\"},\"status\":{\"state\":\"none\"}}");
     // respond to read attachment. since attachment_timeout is 0 in the filter config, just one response is enough
      Envoy::FakeStreamPtr get_stream = sendSquashOk(fake_squash_connection, "{\"metadata\":{\"name\":\"oF8iVdiJs5\"},\"spec\":{\"attachment\":{\"a\":\"b\"},\"image\":\"debug\",\"node\":\"debug-node\"},\"status\":{\"state\":\"attaching\"}}");
...
TEST_P(SquashFilterIntegrationTest, ErrorNoSquashServer) {
    ...
    // Don't respond to anything. squash filter should timeout within squash_request_timeout and continue the request.
    response->waitForEndStream();
